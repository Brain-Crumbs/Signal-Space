...
export function validateScenario(value) {
  ...
  if (nodes.length === 0) errors.push(issue('nodes', 'must contain at least one node'));
  if (!Array.isArray(value.links)) errors.push(issue('links', 'must be an array'));
  ...
  for (const [i, n] of nodes.entries()) {
    ...
    if (n?.response === 'R0' && n.gain !== 0) errors.push(issue(`${p}.gain`, 'must equal 0 when response is R0'));
    if (n?.emission?.law === 'E0') {
      if (!finite(n.emission.nu) || n.emission.nu <= 0) errors.push(issue(`${p}.emission.nu`, 'must be finite and > 0 for E0'));
    } else if (n?.emission?.law === 'E1') {
      if (!finite(n.emission.q) || n.emission.q <= 0) errors.push(issue(`${p}.emission.q`, 'must be finite and > 0 for E1'));
  ...
  const boundaryDelays = ['left', 'right'].map(side => value.boundaries?.[side]).filter(b => b?.kind === 'periodic').map(b => b.closureDelay);
  ...
  for (const side of ['left','right']) {
    ...
    else if (!['open','driven','mirror','periodic'].includes(b.kind)) errors.push(issue(`boundaries.${side}.kind`, 'must select open, driven, mirror, or periodic'));
    if (b?.kind==='driven' && (!finite(b.rate)||b.rate<0)) errors.push(issue(`boundaries.${side}.rate`, 'must be finite and >= 0'));
