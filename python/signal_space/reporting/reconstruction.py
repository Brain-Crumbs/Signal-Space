...
def render_report(run_path,report_path,manifest,analysis,render_provenance):
    ...
    for col,wave in enumerate(waves):
        ...
        axes[1,col].set(title='Last-event neighborhood',xlabel='time (m^-1)',ylabel='a (m)')
    ...
    save(fig,'local-reconstruction','How do the inverse and causal surface capture reproduce the local waveform and its late tail?',
        'Top panels show the complete local linear field; lower panels resolve the threshold neighborhood of the last event.',
    ...
    save(fig,'singular-observability','Which surface singular directions remain relevant to each local event?',
        'The dashed cutoff divides the retained inverse from weakly observed directions. Lower panels show how those directions enter each event field.',
    ...
    save(fig,'event-budgets','Are first and last reconstruction errors separately below their uncertainty budgets?',
        'Each event has its own capture residual budget; small first-event errors cannot hide a last-event discrepancy.',
    ...
    save(fig,'sampling-continuum','How much do sampling and mesh changes move each event?',
        'Lines are adjacent absolute event mesh changes; crosses show output-decimation shifts on the finest grid.',
