...
Reading: Top panels show the complete local linear field; lower panels resolve the threshold neighborhood of the last event.
...
Reading: The dashed cutoff divides the retained inverse from weakly observed directions. Lower panels show how those directions enter each event field.
...
Reading: Each event has its own capture residual budget; small first-event errors cannot hide a last-event discrepancy.
...
Reading: Lines are adjacent absolute event mesh changes; crosses show output-decimation shifts on the finest grid.
