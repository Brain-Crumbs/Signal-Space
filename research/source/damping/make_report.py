...
fine=sorted([r for r in rows if r['dx']==.025 and r['dt']==.005],key=lambda r:r['b_input'])
line=lambda r,n:next(l for l in r['lines'] if l['n']==n)['power_both'][7]
table='\n'.join(f"| {r['b_input']:.4f} | {r['b_measured']:.7f} | {line(r,1):.6e} | {line(r,-2):.6e} | {r['core'][0]['harmonic_energy_decay_rate']:.6e} |" for r in fine)
conv=[]
for tag in ['b0.01_coarse','b0.01_fine','b0.01_timefine','b0.01_superfine']:
 r=next(r for r in rows if r['tag']==tag);d=next(d for d in json.loads((ROOT/'results/discrete_resonance.json').read_text()) if d['dx']==r['dx'] and d['dt']==r['dt'])
 conv.append(f"| {r['dx']:g}, {r['dt']:g} | {-d['Omega_imag']:.9e} | {r['amplitude_gamma_fit']:.9e} | {line(r,1):.7e} |")
balance='\n'.join(f"| {r['b_input']:.4f} | {r['core'][0]['harmonic_excess_loss']:.7e} | {r['core'][0]['excess_loss_fit']:.7e} | {100*abs(r['core'][0]['harmonic_excess_loss']/r['core'][0]['excess_loss_fit']-1):.4f}% |" for r in fine)
report=r'''# Signal Space charged-clock damping milestone
...
![Amplitude scaling, decay rate, convergence and mode comparison](Signal_Space_Charged_Clock_Damping_Diagnostics.png)
'''
report=report.replace('{{SCAN_TABLE}}',table).replace('{{BALANCE_TABLE}}',balance).replace('{{CONVERGENCE_TABLE}}','\n'.join(conv))
(ROOT/'deliverables/Signal_Space_Charged_Clock_Damping_Milestone.md').write_text(report)
