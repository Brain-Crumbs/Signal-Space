...
main=[]
for w in [.5,1,2,4]:
 for nu in [.8,.9,1.]:main.append(D[f'fine_o0.90_w{w:g}_n{nu:g}'])
def row(r):
 a=r['window20'];f=r['profile_fit'];return f"| {r['width_ratio']:g} | {r['nu']:g} | {r['E0_minus_Q0']:.3f} | {a['Q_mean']:.3f} | {a['omega_phase']:.6f} | {100*f['relative_L2_mean']:.2f}% | {a['phase_rms']:.4f} | {'Near branch' if r['width_ratio'] in [1,2] else 'Unsettled at 800'} |"
table='\n'.join(row(r) for r in main)
report=r'''# Signal Space formation milestone: conservative formation and causal phase readout
...
Use the action supplied in *Signal_Space_Ordered_Reception_Charged_Clock_Model.md*:
...
![Formation, charge retention, profile comparison, and phase residual](Signal_Space_Formation_Diagnostics.png)
...
The readout uses two copies of the *evolved*, tapered $T=800$ core, not exact analytic Q-balls. Place their centers at $x=\pm60/m$. For each copy, with $\xi$ its local coordinate, apply
...
![Phase-dependent collisions and the local readout](Signal_Space_Formation_Readout.png)
...
The concrete gain for Signal Space is an autonomous *binding and classical phase recurrence* mechanism within the supplied local field theory, together with an explicit way for formed objects to compare phase by causal contact. The object is not held together by prescribed cavity walls, and its defining recurrence is not an appended decaying shape oscillator.
...
The calculation uses the attached *Signal_Space_Ordered_Reception_Charged_Clock_Model.md* and *Signal_Space_Charged_Clock_Stability_Milestone.md* as its model and benchmark specifications. The attached *Signal_Space_Full_Theory_Research_Architecture-1.md* and *Signal_Space_Part_II.md* supply the distinctions between reference geometry, autonomous binding, clock calibration, and quantum claims.
...
'''
allrows=[];csvrows=[]
for om in [.9,.88,.95,.98]:
 for w in [.5,1,2,4]:
  for nu in [.8,.9,1.]:
   r=D[f'scan_o{om:.2f}_w{w:g}_n{nu:g}'];a=r['window20'];f=r['profile_fit'];allrows.append(f"| {om:.2f} | {w:g} | {nu:g} | {r['E0_minus_Q0']:.3f} | {a['Q_mean']:.3f} | {a['omega_phase']:.6f} | {100*f['relative_L2_mean']:.2f}% | {a['phase_rms']:.4f} |")
   csvrows.append({'target_omega':om,'width_ratio':w,'nu':nu,'E0_minus_Q0':r['E0_minus_Q0'],'core_charge_mean':a['Q_mean'],'phase_frequency':a['omega_phase'],'profile_relative_L2_mean':f['relative_L2_mean'],'phase_residual_rms':a['phase_rms'],'energy_drift':r['energy_drift'],'charge_drift':r['charge_drift']})
report=report.replace('%%MAIN_TABLE%%',table).replace('%%ALL_TABLE%%','\n'.join(allrows))
(OUT/'Signal_Space_Charged_Clock_Formation_Milestone.md').write_text(report)
with (OUT/'formation_scan.csv').open('w') as f:
 w=csv.DictWriter(f,fieldnames=csvrows[0]);w.writeheader();w.writerows(csvrows)
