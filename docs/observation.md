...
6. Sort by reported timestamp, using the fixed permitted-field sequence `side`, `localPhase`, `localFrequency`, `sourceTag` to break ties. Imported object key insertion order is ignored. Distorted reports may lie outside the original gate; no second gate is silently applied.
