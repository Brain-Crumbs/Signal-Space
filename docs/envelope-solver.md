...
To resume, pass the same scenario and envelope options with `resume: snapshot` and an absolute `until >= snapshot.time`. Scenario/options are compared independent of object key order. Restore validates finite dimensions, uninterrupted history from zero, admissible interpolants, endpoint/state agreement, derivatives against the saved causal inputs and equations, every saved RK4 half-step endpoint against deterministic replay, each retained pair against the original coarse-versus-half-step and dense-midpoint acceptance tests, the exact solver-boundary partition, and the adaptive controller's pair-to-pair growth and saved next-step limits (smaller proposals remain valid because rejected trials may shrink them). Deleting history, changing parameters or accumulated intensities, substituting individually valid but tolerance-rejected RK4 steps, crossing a scheduled discontinuity, or inserting a solver-inaccessible step-size jump does not produce a valid restart. Declared future gain/input changes are already retained in the scenario/options.
