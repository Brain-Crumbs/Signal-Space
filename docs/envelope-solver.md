...
- Accepted history is checked for finite values and finite extrema calculations, positive and physically bounded frequency throughout its interpolant, monotone phase and nondecreasing integrated intensities. Solver error tolerances never widen the physical frequency interval; only an eight-ULP allowance covers representation roundoff in interpolant calculations. Out-of-bound trials shrink the step. No phase/frequency clipping occurs. Step underflow, unrepresentable tick indices, overflow and exhausted attempt budgets produce `NUMERICAL_FAILURE`. The default lifetime budget is 100,000 accepted plus rejected steps, configurable through `maxSteps` up to 1,000,000.
- Ticks obey `(t0,t]` and the supplied section (default 0 rad). Each monotone phase cubic is searched by 52 bisection iterations; root accuracy is limited by floating-point resolution and trajectory error. At most 10,000 crossings per node per half step are emitted. Huge unwrapped phases whose exact next-higher floating-point spacing cannot resolve one section, whose reconstructed section does not contain the phase, or whose indices are not usable fail explicitly.
- Source startup, driven-input changes, gain interventions and explicit-history knots propagate through four transport generations as exact step endpoints (including empty-link first arrivals). RK endpoint derivatives use the left limit; the next step and reported sample use the right limit. Adjacent boundary representations with no floating-point value between them are aligned to avoid artificial vanishing steps when equivalent expressions round differently; direct event activation times are canonicalized to the same retained endpoint. Resolvable intervals, including multi-ULP intervals and intervals near zero, remain distinct. Each feedback integration raises differentiability by one; later generations are handled by adaptive error control. More than 1,000,000 scheduled boundary entries fail explicitly.

## Preparation and supported interventions

The default established preparation uses the scenario's constant admissible frequency and linear phase over its declared history interval. `envelope.prehistory` optionally supplies strictly increasing `{time, nodes: {id: {phi, omega}}}` knots covering that interval through zero. They define a cubic Hermite **phase function**, with frequency given by its derivative. Its endpoint must match the scenario and its entire derivative must lie in the physical admissible frequency interval, apart from the same eight-ULP representation allowance used for evolved interpolants; solver error tolerances do not widen this input-domain bound. Sampling a history function into this data format keeps requests and checkpoints structured-cloneable; executable callbacks are not accepted.
...
To resume, pass the same scenario and envelope options with `resume: snapshot` and an absolute `until >= snapshot.time`. Scenario/options are compared independent of object key order. The snapshot records every accepted and rejected adaptive attempt. Restore deterministically replays that trace, including each controller outcome and resulting next-step proposal; checks finite dimensions, uninterrupted history from zero, admissible interpolants, endpoint/state agreement, RK4 half-step state and derivative reproduction, coarse-versus-half-step and dense-midpoint acceptance, the solver-boundary partition, and accepted-plus-rejected counters against the lifetime attempt budget. Replayed floating-point values allow at most `1e-12` absolute drift or 32 representable IEEE-754 values measured from their ordered bit patterns, matching the documented cross-runtime contract for last-bit differences without allowing the bound to grow into a macroscopic physical-state change; every saved segment and the final endpoint remain tied to one trusted replay. A saved acceptance decision may differ from replay only when the replayed normalized error is within `1e-12` of the acceptance threshold; the saved decision still determines whether a complete segment pair must exist. Deleting history, changing parameters or accumulated intensities, cumulatively perturbing otherwise sub-threshold increments, forging rejection counters, substituting tolerance-rejected RK4 steps, crossing a scheduled discontinuity, inserting a solver-inaccessible step-size jump, or exceeding the declared attempt budget does not produce a valid restart. Declared future gain/input changes are already retained in the scenario/options.
