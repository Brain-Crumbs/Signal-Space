...
To resume, pass the same scenario and envelope options with `resume: snapshot` and an absolute `until >= snapshot.time`. Scenario/options are compared independent of object key order. Restore validates finite dimensions, uninterrupted history from zero, admissible interpolants, endpoint/state agreement, derivatives against the saved causal inputs and equations, and every saved RK4 half-step endpoint against deterministic replay. Deleting history or changing parameters or accumulated intensities in the checkpoint does not produce a valid restart. Declared future gain/input changes are already retained in the scenario/options.
