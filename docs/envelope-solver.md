...
- Ticks obey `(t0,t]` and the supplied section (default 0 rad). Each monotone phase cubic is searched by 52 bisection iterations; root accuracy is limited by floating-point resolution and trajectory error. At most 10,000 crossings per node per half step are emitted. Huge unwrapped phases whose floating-point spacing cannot resolve one section, whose reconstructed section does not contain the phase, or whose indices are not usable fail explicitly.
- Source startup, driven-input changes, gain interventions and explicit-history knots propagate through four transport generations as exact step endpoints (including empty-link first arrivals). RK endpoint derivatives use the left limit; the next step and reported sample use the right limit. Boundary times and endpoint arithmetic within eight machine epsilons are aligned to avoid artificial vanishing steps when equivalent floating-point expressions land on adjacent representations; direct event activation times are canonicalized to the same retained endpoint. Each feedback integration raises differentiability by one; later generations are handled by adaptive error control. More than 1,000,000 scheduled boundary entries fail explicitly.
